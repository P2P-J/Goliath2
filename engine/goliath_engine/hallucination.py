...
#:
#: 낮게 잡는 이유: VAD 는 발화 프레임의 56~59% 만 잡는다 (실측). 음절 사이의
#: 짧은 정지를 무음으로 표시하기 때문이며 버그가 아니라 VAD 의 본성이다.
#: 따라서 speech_sec 는 실제 발화 길이의 약 57% 로 나온다 — "반가워"(0.9초)가
#: 0.4초로 집계된다. 0.4 로 잡으면 1초 미만의 짧은 명령이 전부 폐기된다.
#:
#: 2프레임(0.16초)이면 소음 오발동(1프레임)은 걸러지고 "정지", "다음" 같은
#: 짧은 명령은 통과한다. 실측에서 소음 오탐율은 임계 0.3~0.5 모두 0.0% 였으므로
#: 여기서 느슨해도 안전하다. 소음에서 나온 쓰레기 인식은 인식 후 판정
#: (신뢰도·블랙리스트)이 잡는다.
MIN_SPEECH_SEC = 0.16
#: 발화로 판정된 프레임 비율의 하한.
#:
